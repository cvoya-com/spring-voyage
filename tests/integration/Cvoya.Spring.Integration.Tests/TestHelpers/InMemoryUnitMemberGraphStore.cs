...
/// integration-style unit tests exercise the EF-backed member graph
/// (#2052 / ADR-0040) without standing up Postgres / a Testcontainer.
/// Mirrors the corresponding helper in the Dapr unit-test project.
/// </summary>
public class InMemoryUnitMemberGraphStore : IUnitMemberGraphStore
{
    ...
    /// <summary>
    /// Test convenience: seed agent edges directly.
    /// </summary>
    ...
    /// <summary>
    /// Test convenience: seed sub-unit edges directly.
    /// </summary>
    ...
    /// <summary>
    /// Test convenience: seed members for a unit by Address. Works for
    /// both agent-scheme and unit-scheme addresses.
    /// </summary>
    public void SeedMembers(Guid unitId, params Address[] members)
    {
        foreach (var member in members)
        {
            if (string.Equals(member.Scheme, Address.AgentScheme, StringComparison.OrdinalIgnoreCase))
            {
                SeedAgentMembers(unitId, member.Id);
            }
            else if (string.Equals(member.Scheme, Address.UnitScheme, StringComparison.OrdinalIgnoreCase))
            {
                SeedSubunitChildren(unitId, member.Id);
            }
        }
    }
}
