#!/usr/bin/env bash
# pool: fast
# `spring agent create --inherit` sends no definitionJson execution block.
set -euo pipefail
...
e2e::log "spring agent create --name ada-inherit --unit ${unit_id} --inherit"
response="$(e2e::cli_agent_create --name ada-inherit --unit "${unit_id}" --inherit)"
code="${response##*$'\n'}"

e2e::expect_status "0" "${code}" "agent create with full inherit succeeds"
...
PY
    ...
    e2e::ok "agent create inherit request omits definitionJson"
else
    e2e::fail "agent create inherit request mismatch: ${validation_error}"
fi
