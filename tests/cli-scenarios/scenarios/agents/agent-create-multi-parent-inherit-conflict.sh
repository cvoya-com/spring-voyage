#!/usr/bin/env bash
# pool: fast
# `spring agent create --inherit` surfaces the structured ADR-0039
# multi-parent inheritance conflict from the public API.
set -euo pipefail
...
e2e::log "spring agent create --name ada-conflict --unit ${unit_a} --unit ${unit_b} --inherit"
response="$(e2e::cli_agent_create --name ada-conflict --unit "${unit_a}" --unit "${unit_b}" --inherit 2>&1 || true)"
code="${response##*$'\n'}"
body="${response%$'\n'*}"
...
if len(payload.get("unitIds", [])) != 2:
    raise SystemExit(f"expected two unitIds, got {payload.get('unitIds')}")
definition = payload.get("definitionJson")
if definition not in (None, ""):
    raise SystemExit(f"expected no definitionJson, got {definition!r}")
PY
)"; then
    e2e::ok "multi-parent inherit request sends no explicit execution config"
else
    e2e::fail "multi-parent inherit request mismatch: ${validation_error}"
fi
