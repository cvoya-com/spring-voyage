...
e2e::log "spring agent create --name ada-test --unit ${unit_id} --description 'A helpful agent'"
response="$(e2e::cli_agent_create --name ada-test --unit "${unit_id}" --description "A helpful agent")"
code="${response##*$'\n'}"

e2e::expect_status "0" "${code}" "agent create with description succeeds"

if grep -Fq '"description":"A helpful agent"' "${request_log}"; then
    e2e::ok "stub API received the description in the create request body"
else
    e2e::fail "stub API did not receive the expected description body: $(<"${request_log}")"
fi
