#!/usr/bin/env bash
# pool: fast
# `spring agent create` with full execution overrides sends the expected
# definitionJson execution block through the public API.
set -euo pipefail
...
e2e::log "spring agent create --name ada-overrides --unit ${unit_id} --runtime claude-code --model-provider anthropic --model claude-opus-4-7 --image ghcr.io/example/agent:latest"
...
e2e::expect_status "0" "${code}" "agent create with full execution overrides succeeds"
...
if payload.get("displayName") != "ada-overrides":
    raise SystemExit(f"unexpected displayName: {payload.get('displayName')}")
if len(payload.get("unitIds", [])) != 1:
...
if execution != expected:
    raise SystemExit(f"unexpected execution block: {execution}")
PY
)"; then
    e2e::ok "agent create request carries full execution override definitionJson"
else
    e2e::fail "agent create full override request mismatch: ${validation_error}"
fi
