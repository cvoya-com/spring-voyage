#!/usr/bin/env bash
# pool: fast
# `spring agent create --from-package --connector` routes connector bindings
# through the public package install API.
set -euo pipefail
...
response="$(e2e::cli_agent_create --name package-agent --from-package my-package --connector github=b1)"
code="${response##*$'\n'}"

e2e::expect_status "0" "${code}" "agent create from package with connector succeeds"
...
target = json.loads(body)["targets"][0]
if target.get("packageName") != "my-package":
    raise SystemExit(f"unexpected packageName: {target.get('packageName')}")
binding = target["connectorBindings"]["package"]["github"]
if binding.get("config") != {"bindingId": "b1"}:
    raise SystemExit(f"unexpected connector binding: {binding}")
PY
)"; then
    e2e::ok "package install request carries connector binding"
else
