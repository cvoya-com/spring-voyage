#!/usr/bin/env bash
# pool: fast
# `spring agent create` without --unit creates a tenant-parented agent request.
set -euo pipefail
...
e2e::log "spring agent create --name ada-top-level --inherit"
response="$(e2e::cli_agent_create --name ada-top-level --inherit)"
code="${response##*$'\n'}"

e2e::expect_status "0" "${code}" "top-level agent create succeeds"
...
PY
    ...
    e2e::ok "top-level agent request sends empty unitIds and no definitionJson"
else
    e2e::fail "top-level agent request mismatch: ${validation_error}"
fi
