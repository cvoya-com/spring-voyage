...
public class PersistentAgentCommandTests
{
    ...
    public void AgentDeploy_ParsesImageAndReplicasOptions()
    {
        ...
        var parseResult = root.Parse(
            "agent deploy ada --image ghcr.io/cvoya-com/spring-voyage-claude-code-base:2.1.98 --replicas 1");

        parseResult.Errors.ShouldBeEmpty();
        parseResult.GetValue<string>("id").ShouldBe("ada");
        parseResult.GetValue<string>("--image").ShouldBe("ghcr.io/cvoya-com/spring-voyage-claude-code-base:2.1.98");
