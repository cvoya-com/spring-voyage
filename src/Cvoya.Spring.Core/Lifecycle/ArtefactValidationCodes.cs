...
/// workflow emits one of these as the <see cref="ArtefactValidationError.Code"/>
/// value so UI, CLI, and log consumers can branch on a stable, machine-readable
/// identifier rather than the free-form <see cref="ArtefactValidationError.Message"/>.
/// Values are intentionally equal to the constant names.
/// </summary>
public static class ArtefactValidationCodes
{
    ...
    /// <summary>The probe failed with an unexpected internal error; details should be attached on <see cref="ArtefactValidationError.Details"/>.</summary>
    ...
    /// to run. <see cref="ArtefactValidationError.Details"/> carries the missing
