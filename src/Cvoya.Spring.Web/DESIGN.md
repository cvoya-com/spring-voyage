...
1. **Agents may be tenant-parented or unit-parented.** `POST /api/v1/tenant/agents` accepts `unitIds: []` for a top-level tenant-parented agent, or one-or-more unit ids for membership at create time. Multi-parent membership is allowed — an agent can belong to several units simultaneously. Two portal surfaces follow this rule: the standalone `/agents/create` page (`src/app/agents/create/page.tsx`) and the unit Agents-tab `Add agent` dialog (`AgentCreateDialog`, wrapping the shared `<AgentCreateForm>`). Both funnel through `src/lib/agents/create-agent.ts`, which owns the direct create body assembly (`displayName`, `description`, `role`, `unitIds`, nullable `definitionJson`) so the on-disk agent record matches one written by `spring agent create`. Per-membership edits use the edit-only `<MembershipDialog>`; the old add-mode picker is not part of the Agents tab flow.
...
Humans are reached from a dedicated `/humans/<guid>` route (not from the tenant-tree's left rail — humans are not part of the tree payload in v0.1). The Explorer's `?node=human:<guid>` selection bounces to `/humans/<guid>` via `router.replace`, so Cmd-K teleport, activity-feed `human:` rows, and unit-membership rows from #2270 + #2427 funnel through one entry point. The Detail Pane chrome is the same `<DetailPane>` the Explorer mounts — same address-copy affordance, same tab strip — minus the lifecycle status badge (humans don't have a runtime lifecycle).
...
| Messages   | Slot reserved for #2268 (Portal Wave B). Renders `<TabPlaceholder kind="Human" tab="Messages">` until the body ships — same chrome the registry uses for any unregistered slot. The eventual body is the canonical `<MessagesTab kind="Human" id={...} />` filtered to threads the human is addressed in. |
| **Config** (overflow) | Slot reserved for #2269 (Portal Wave B). Renders `<TabPlaceholder kind="Human" tab="Config">` until the body ships. The eventual body has two sub-tabs: **Identity** (name, email, display preferences) and **Connector** (the inbound-routing binding — Slack handle, GitHub handle, email — that lets the platform deliver `human:<id>`-addressed messages via the right channel). |
