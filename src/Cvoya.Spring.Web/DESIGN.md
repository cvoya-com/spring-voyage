...
`GET /api/v1/tenant/tree` returns the entire synthesized tenant tree in one response (no pagination, no lazy expand). The tree carries Tenant, Unit, Agent, and Human nodes — humans (#2466) render under every unit they are a team-role member of, with the canonical `human://<guid>` id so the Explorer's `human:` redirect routes clicks to the dedicated `/humans/<guid>` page. The frontend calls it via `useTenantTree()` (`src/lib/api/queries.ts`) and pipes the result through `validateTenantTreeResponse` so stray `kind` / `status` values from the server coerce to safe defaults before the Explorer renders.
...
Humans are reached from a dedicated `/humans/<guid>` route. The tenant-tree payload now renders a Human child under every unit a human is a team-role member of (#2466), so the left-rail tree is also a reachable entry point — duplicates across units are intentional (one row per `(unit, human)` membership) and clicking any of them lands on the same `/humans/<guid>` page. The Explorer's `?node=human:<guid>` selection bounces to `/humans/<guid>` via `router.replace`, so Cmd-K teleport, activity-feed `human:` rows, tree-row clicks, and unit-membership card clicks from #2270 + #2427 funnel through one entry point. The Detail Pane chrome is the same `<DetailPane>` the Explorer mounts — same address-copy affordance, same tab strip — minus the lifecycle status badge (humans don't have a runtime lifecycle).
