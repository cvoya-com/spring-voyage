...
**Slack workspace panel (#2820, ADR-0061).** The Slack connector binds at **tenant scope** (not per-unit), so its surface lives as a `/settings` drawer-panel rather than a tab inside the Explorer's Connector tab. `<SlackConnectorPanel>` lives inside the connector package at `src/Cvoya.Spring.Connector.Slack/web/` and is registered via the `@connector-slack/*` tsconfig alias — same shape the GitHub connector's unit-scope tab uses. The panel has three states:

- **Empty** — short explainer + a primary `Install in Slack workspace` button that routes (`router.push`) to the one-page install wizard at `/connectors/slack/install` (#2882). The empty state no longer starts OAuth inline: a fresh tenant has no Slack app yet, so it goes through the wizard, which registers the app first.
- **Bound** — `Connected` badge + workspace name / team id / bot user / installer Slack user id / bound TenantUser id / connected-since timestamp, then a row with `Reconnect` (outline) and `Disconnect` (destructive). `Reconnect` re-runs OAuth inline (the app + credentials already exist): it opens a popup, POSTs `/api/v1/tenant/connectors/slack/oauth/authorize`, navigates the popup to the consent URL, and resolves on the `sv:slack:oauth:done` postMessage handoff (#2837). The disconnect button opens a `<ConfirmDialog>` whose Cancel button is first in tab order so the dialog's focus-first-focusable behaviour lands on Cancel, not on the destructive confirm action. The confirmation invokes `POST /api/v1/tenant/connectors/slack/disconnect` which revokes the bot token via Slack's `auth.revoke` and deletes the binding + tenant secrets in one server-side transaction (ADR-0061 §2.5).
- **Error states** (reconnect path) — three palettes: `enterprise-grid` (warning palette, ADR-0061 §2.4 link, no retry — operator must use a non-Grid workspace), `not-configured` (info palette, copy points at `spring.env` keys, no retry), `generic` (destructive palette with a `Try again` CTA back into the OAuth start). The Enterprise-Grid case keys off `code = SlackEnterpriseGridUnsupported` on the ProblemDetails body, so a future copy change on the server still routes to the right banner.

v0.1 limitations (single bound user, DM-only operation) surface in panel copy with a link to ADR-0061. The CLI mirror is `spring connector slack …`.

**Slack install wizard (#2882, ADR-0061 §2.5).** `/connectors/slack/install` (`src/app/connectors/slack/install/page.tsx`) is the one-page, no-CLI install surface the empty-state CTA routes to. It eliminates the `spring connector slack install` round-trip from the common case: the operator pastes a Slack **Configuration Token**, optionally overrides the app name / SV host and ticks **Socket Mode**, and clicks **Install in Slack**. Two actions back the same `POST /api/v1/tenant/connectors/slack/install` endpoint:
...
The replay engine reuses the live pulse animation pipeline — the rewind component dispatches per-pulse frames through the same `onPulse` callback the SSE handler feeds, so the graph dot motion and the matrix cell flash are identical. The timeline brush keeps its snapshot-mode semantics in rewind: dragging either edge updates `[since, until]` in the URL and the rewind transport replays through whatever window is currently selected. Cursor position is separate — a dashed `<ReferenceLine>` painted in `--color-primary` sits at the playback head, and clicking a bucket on the chart seeks the cursor to that bucket's start (rewind-only; clicks in snapshot / live mode fall through). Speed-change and scrub events rebase the realtime anchor so the cursor never jumps mid-burst. Each dispatched rewind pulse gets a fresh per-dispatch id (the graph dedupes by id to absorb SSE reconnects, which would otherwise filter the second-and-later playthroughs of the same `messageId`).
...
- **`dialog`** (default — passed when the active user is a participant). Chat-style bubbles aligned by sender role: human → right (`bg-blue-600 text-white` — fixed blue-600 so the pair passes WCAG AA in both themes; #2516 / #2770), every other role → left (`bg-muted`). Tool calls and lifecycle events render as collapsible call-outs inside the same bubble column. This metaphor reads as a conversation with the user on one side and the agents/units on the other.
