...
1. **Every agent has at least one parent unit.** The agent create / update API rejects empty `unitIds` with a 400. Multi-parent membership is allowed — an agent can belong to several units simultaneously. Two portal surfaces respect this rule client-side so the API never sees a doomed request: the standalone `/agents/create` page (`src/app/agents/create/page.tsx`) and the unit Agents-tab `Add agent` dialog (`AgentCreateDialog`, wrapping the shared `<AgentCreateForm>`). Both funnel through `src/lib/agents/create-agent.ts`, which owns the URL-safe id pattern, the `unitIds.length > 0` rule, and the `definitionJson` body assembly so the on-disk agent record is byte-identical to one written by `spring agent create`. Per-membership edits still use `<MembershipDialog mode="edit">`; the old add-mode picker is not part of the Agents tab flow.
