...
- There is **no standalone "Validate" button** (#655). Validation fires from the wizard's Next handler: clicking Next on the credential step auto-posts the typed key to `POST /api/v1/system/credentials/{provider}/validate`, and advances only on a valid verdict. The Next button's label flips to `Validating…` while the call is in flight, and stays on the step with the inline error when the provider rejects the key.
- Validation result lives directly under the input, not inside the outer banner. In-flight is `text-xs text-muted-foreground` ("Validating &lt;Provider&gt; API key…"). Success is `text-xs text-emerald-700 dark:text-emerald-300` with a `CheckCircle2` lead. Failure is `text-xs text-destructive` with an `AlertTriangle` lead and the server-supplied operator message verbatim. Editing the key clears the verdict so the next Next click re-validates.
