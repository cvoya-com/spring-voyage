...
- A **Validate** button (#655) sits inline with the Show/Hide toggle. `<Button type="button" size="sm">` — `variant="default"` when the current key hasn't been validated yet, `variant="outline"` (labelled **Re-validate**) once the provider has accepted it. Disable while the mutation is in flight (label `Validating…`). Editing the key clears the verdict so the label reverts to **Validate** and the Next-step gate re-engages.
- Validation result lives directly under the input, not inside the outer banner. Success is `text-xs text-emerald-700 dark:text-emerald-300` with a `CheckCircle2` lead; failure is `text-xs text-destructive` with an `AlertTriangle` lead and the server-supplied operator message verbatim. The idle-with-typed-key hint is `text-xs text-muted-foreground` ("Click Validate to confirm the key works before continuing.") so operators learn the gate exists before they click Next.
