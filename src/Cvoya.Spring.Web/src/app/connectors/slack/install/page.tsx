...
import { ArrowLeft, ExternalLink, Loader2, Slack } from "lucide-react";
...
export default function SlackInstallWizardPage() {
  ...
  const busy =
    status === "previewing" || status === "installing" || status === "awaiting";
  ...
  const install = useCallback(async () => {
    ...
    // user-initiated window — many browsers block `window.open` after an
    // `await`. We navigate it once the authorize URL resolves.
    ...
    try {
      ...
      authorizeUrl = res.authorizeUrl ?? null;
    ...
    if (authorizeUrl === null || authorizeUrl === "") {
      // The app + credentials were created but no consent URL came back.
      // The credentials are persisted, so point the operator at the
      // settings panel to finish the OAuth consent via Reconnect.
      popup.close();
      popupRef.current = null;
      setError({
        title: "Slack app created, but no consent URL was returned",
        detail:
          "The OAuth credentials were saved. Go to Settings → Slack workspace and click Reconnect to finish the consent step.",
      });
      setStatus("failed");
      return;
    }
    ...
    if (outcome.kind === "success") {
      toast({
        ...
        description:
          "The app is registered and the bot can now post DMs for this tenant.",
      });
    ...
    setError({
      ...
      detail:
        outcome.kind === "popup-closed"
          ? "The Slack window closed before the install finished. The app and credentials may already be saved — check Settings → Slack workspace, or click Install in Slack to retry."
          : outcome.kind === "timed-out"
            ? "The Slack window stayed open too long. The app and credentials may already be saved — check Settings → Slack workspace, or click Install in Slack to retry."
            : "The install was interrupted. Check Settings → Slack workspace, or click Install in Slack to retry.",
    });
    setStatus("failed");
  }, [buildBody, configToken, router, toast]);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6" data-testid="slack-install-wizard">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">1. Slack app details</CardTitle>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">2. Review &amp; install</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
