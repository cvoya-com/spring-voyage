...
/// <summary>
/// Output of the <see cref="ArtefactValidationWorkflow"/>: a boolean verdict,
/// the originating <see cref="ArtefactValidationError"/> on failure, and an
...
/// <param name="Failure">Structured failure payload — non-<c>null</c> iff <see cref="Success"/> is <c>false</c>. Carries the step that failed, a stable <see cref="ArtefactValidationCodes"/> code, an operator message, and optional structured details.</param>
/// <param name="LiveModels">Live model catalog parsed from the <see cref="ArtefactValidationStep.ResolvingModel"/> step's <see cref="Cvoya.Spring.Core.ModelProviders.StepResult.Extras"/> (key <c>"models"</c>, comma-separated) when the runtime's interpreter emitted one; <c>null</c> on failure or when the runtime does not enumerate a live catalog (e.g. the Claude CLI probe only confirms the requested model).</param>
public record ArtefactValidationWorkflowOutput(
    bool Success,
