...
**Active release frame.** First public release of Spring Voyage — the source-available collaboration platform for teams of AI agents and the humans they work with. Replaces the scrapped V2 release. V1 was internal-only.
...
Three v0.1 blockers surfaced mid-flight and do not map onto a single A–J area; each is tracked under its own umbrella:

- **[#1554](https://github.com/cvoya-com/spring-voyage/issues/1554) — Package as the unit of bundling, installation, and export.** Collapses the *template* / *package* split, lands atomic multi-unit install, and rewires the new-unit / new-agent wizards onto one install pipeline. Forcing function: [#1530](https://github.com/cvoya-com/spring-voyage/issues/1530) (the OSS dogfooding package's README promises capabilities the v0.1 install path does not yet have). Locked in [ADR-0035](../../../decisions/0035-package-as-bundling-unit.md). Schema decomposition follow-on locked in [ADR-0037](../../../decisions/0037-package-schema-decomposition.md) — every YAML kind-discriminated, per-artefact `requires:`, versioned cross-package addressing, no `inputs:` / package-level `connectors:`. Implementation tracked under [#1722](https://github.com/cvoya-com/spring-voyage/issues/1722) (sub-issues #1724–#1727).
...
- **[#2487](https://github.com/cvoya-com/spring-voyage/issues/2487) — TenantUser / human split + connector identity location.** Introduces the `TenantUser` actor kind (per-tenant; same auth subject in two tenants is two distinct rows), relocates display-side connector identity off `Human` onto a shrunk `TenantUserConnectorIdentity` row (no auth fields), and lands binding-side auth on `UnitGitHubConfig` — `Owner` removed, `AppInstallationId` kept, `PatSecretName` added, exactly one of the two required at binding-create time. The binding's pinned credential is what every outbound call uses regardless of caller. Within a tenant, many bindings may target the same `(owner, repo)` and the webhook handler fans out (cross-tenant `(owner, repo)` collision still rejected at create time). Ships OAuth-issued PAT acquisition as a v0.1 in-scope flow that produces a tenant secret + optionally populates display identity. Supersedes the original single-PR shape of #2487 (a localised webhook fix) and pulls forward `TenantUser` pieces originally scoped for v0.2 / v0.3. Locked in [ADR-0047](../../../decisions/0047-platform-user-human-split.md); execution plan in [`platform-user-split.md`](platform-user-split.md).
