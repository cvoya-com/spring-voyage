...
- **Orchestration tools are attached uniformly.** The platform exposes a closed set of orchestration action verbs (`delegate_to`, `fanout_to`) to every `agent://` and `unit://` runtime, via (a) the runtime's tool-call surface (MCP / env-var-keyed, for LLM-driven runtimes) and (b) a typed SDK over an HTTP callback API (`Cvoya.Spring.AgentSdk`, for workflow-driven runtimes). Discovery, inspection, and runtime-status queries live on the `sv.*` directory tool surface. Both action-verb surfaces share the same handlers and emit the same `OrchestrationDecision` events. The image author chooses which fits.
