...
- **Messaging tools are attached uniformly.** The platform exposes the `sv.messaging.send` / `sv.messaging.broadcast` delivery tools to every `agent://` and `unit://` runtime, via (a) the runtime's tool-call surface (MCP, for LLM-driven runtimes) and (b) a typed SDK over an HTTP callback API (`Cvoya.Spring.AgentSdk`, for workflow-driven runtimes). Discovery, inspection, and runtime-status queries live on the `sv.directory.*` tool surface. `delegate_to` / `fanout_to` are removed — see [ADR-0050](../../decisions/0050-platform-mcp-tool-surface.md). The image author chooses which surface fits.
