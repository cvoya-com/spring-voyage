...
4. **Deployment story is not meaningfully worse.** The shipped `eng/build/Dockerfile` already runs Node to serve the portal — this was the case before and after the flip. The ADR's original worry ("every self-hoster needs a Node process") was already invalidated by the existing container topology.
