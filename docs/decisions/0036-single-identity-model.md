...
- **Related ADRs:** [0023 — flat actor ids](0023-flat-actor-ids.md) — the predecessor this ADR amends. The single-hop routing decision in 0023 is unchanged; only the identifier *type* and *form* are tightened. [0017 — Unit is an agent composite](archive/0017-unit-is-an-agent-composite.md) — the composite pattern this identity model preserved (archived; superseded by [0070](0070-subjects-agents-humans-teams.md)). [0030 — thread model](0030-thread-model.md) — participants are addressed by Guid identity, not by display name. [0035 — package as bundling unit](0035-package-as-bundling-unit.md) — cross-package reference grammar uses Guids only.
- **Related docs:** [`docs/architecture/messaging.md`](../architecture/messaging.md), [`docs/architecture/units.md`](../architecture/units-and-agents.md), [`docs/architecture/identifiers.md`](../architecture/data-and-identity.md) (forthcoming under #1633).
...
> **Amended by [ADR-0070](0070-subjects-agents-humans-teams.md) (2026-07-31):** the kind enumeration reshapes — `unit` is eliminated, and **team** is introduced as a subject kind that is *not* an actor (no mailbox, no address scheme on the routable set; teams are organizational, policy, and resource scopes). Everything else here is unchanged and load-bearing for 0070: Guid identity, presentation-only `display_name`, and the membership graph as the addressing fabric — the graph re-roots on teams under the tenant.
