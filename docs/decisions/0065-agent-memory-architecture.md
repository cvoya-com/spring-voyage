...
- `sv.memory.*` content is a **structured JSON value** stored in `jsonb` (#2991): a plain note is a JSON string, structured state is an object/array, round-tripped type-preserved through `add` / `get` / `list` / `search` / `update` (Postgres `to_tsvector(jsonb)` makes structured entries searchable by their string values). Agents record structured working state — the "edition status board" — without hand-stringifying; there is no separate format discriminator (the JSON type is the discriminator).
