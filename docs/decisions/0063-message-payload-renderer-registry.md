...
- **Explicit `payload.kind` discriminator (option 2 in #2843).** This is the cleaner long-term shape — producers self-declare via `kind: "chat.text" | "a2a.task" | …` and renderers select by exact key. Deferred: requires coordinated producer-side changes the v0.1 surface can absorb later without disturbing this registry. The follow-up issue captures the producer-site migration story; the registry's interface already accommodates a `KindKeyedRenderer` once the producer side starts emitting `kind`.
...
- **Future work (deferred).** Converge the A2A reasoning-trace extractor with the renderer registry by routing A2A responses through a `Message`-shaped intermediate; pin the explicit `payload.kind` discriminator (option 2 from #2843) when the producer-side migration is scoped.
