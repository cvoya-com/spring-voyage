...
- **A2A convergence (#2856).** The follow-up landed: `A2AExecutionDispatcher` wraps each `AgentTask` / `AgentMessage` response into a `Message.Payload`-shaped envelope with a namespaced `kind` (`"a2a.task"` / `"a2a.message"`) and routes it through the registry. The new `A2aTaskPayloadRenderer` walks artifacts → status.message → history, mirroring the retired in-file extractor. The `kind` field here is **producer-side and internal** — the dispatcher is a Spring-Voyage component wrapping an external A2A response at the platform boundary, which is exactly the "wrap them at the boundary regardless" pattern the rejected alternative endorsed. It is **not** a re-introduction of the platform-wide producer-self-declaration scheme that alternative rejected.
