...
- **Boundary evaluation on read by the aggregator consumer.** Pros: the aggregator returns the raw view and each consumer (HTTP endpoint, MCP, runtime orchestration tools) applies its own filtering. Cons: rules get duplicated across call sites; a new consumer that forgets the filter leaks opaque entries. Rejected — one decorator, one set of rules.
