...
> **Partially superseded by [ADR-0070](0070-subjects-agents-humans-teams.md) (2026-07-31).** The unit-as-agent portions — §1 (a unit is an agent), the unit framing of §2, and §6 (execution-config inheritance) — are superseded: the unit concept is eliminated; teams are recursive organizational, policy, and resource scopes that are **not** participants, and execution config is intrinsic to the agent. The one-way delivery substrate (§§3–5 — one-way events, delivery-acknowledgement tools, the fast-enqueue invariant, non-routable connectors) **stands**, pending the interactions companion record ([#3249](https://github.com/cvoya-com/spring-voyage/issues/3249)).

- **Status:** Accepted — 2026-05-22. **Re-baseline.** Unit-as-agent portions superseded by [0070](0070-subjects-agents-humans-teams.md) — see the banner above. This record restates the current design directly. It consolidates and supersedes the archived [0039](archive/0039-units-are-agents.md) (units are agents), [0048](archive/0048-event-vs-request-message-semantics.md) (one-way domain messaging), and [0049](archive/0049-message-delivery-tool-contract.md) (delivery-acknowledgement contract). Those three ADRs recorded the design as it evolved, in fast succession, away from a platform-owned orchestration layer; the live decision ended up spread across them with amendment headers and strike-through bodies. Nothing here reverses 0039 / 0048 / 0049 — it states what they collectively decided, cleanly. The archived records keep the full rejected-alternatives reasoning.
- **Date:** 2026-05-22
- **Related ADRs:** [0054](0054-one-mcp-server-one-execution-host.md) — the platform MCP surface and execution hosts; the `sv.messaging.send` / `sv.messaging.multicast` tools named below are defined there. [0017](archive/0017-unit-is-an-agent-composite.md) — the composite pattern this builds on (archived; superseded by [0070](0070-subjects-agents-humans-teams.md)). [0038](0038-agent-runtime-and-model-provider-split.md) — the `(runtime, model)` configuration a unit and a leaf agent share. [0030](0030-thread-model.md) — the participant-set thread the dispatch response is recorded on. [0045](0045-connector-domain-agnostic-platform.md) — connectors facilitate flow but do not replicate upstream config.
