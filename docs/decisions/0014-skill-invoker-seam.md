...
> **Withdrawn — [#2989](https://github.com/cvoya-com/spring-voyage/issues/2989).** Kept for reasoning history; it no longer describes the current system. The `ISkillInvoker` seam existed solely to dispatch the `sv.expertise.{slug}` capability tools and the `sv.expertise.search` meta-skill through `IMessageRouter` with a caller carried in the request body. #2989 unifies the platform tool surface on the caller-aware `ToolCallContext` path and removes those expertise tools — an agent now discovers peers with a given expertise through the caller-aware `sv.directory.list` / `sv.directory.lookup` tools (the `expertise` filter + per-entry expertise list). With the only caller and the only implementation gone, `ISkillInvoker`, `SkillInvocation`/`SkillInvocationResult`, `MessageRouterSkillInvoker`, `IExpertiseSkillCatalog`, `ExpertiseSkillCatalog`, `ExpertiseSkill`, `ExpertiseSkillRegistry`, and `DirectorySearchSkillRegistry` were deleted. The expertise *directory* itself (`IExpertiseAggregator`, `IExpertiseStore`, `IExpertiseSearch`, the HTTP `POST /api/v1/directory/search` endpoint) is unchanged. The future A2A gateway (#539), if it lands, will introduce its own caller-aware seam rather than revive this one.

- **Status:** Withdrawn (was Accepted) — see the banner above. ~~`ISkillInvoker` is the single abstraction every skill caller (planners, the MCP server, future A2A gateway) resolves. The default implementation, `MessageRouterSkillInvoker`, translates a `SkillInvocation` into a `MessageType.Domain` `Message` and routes it through `IMessageRouter` so the whole platform governance chain (boundary, permissions, policy, activity) runs end-to-end. Callers never see `Message`.~~
- **Date:** 2026-04-17
