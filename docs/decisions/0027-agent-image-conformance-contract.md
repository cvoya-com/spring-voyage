...
| **2** | Pull the bridge into a custom base by copying the per-target SEA binary from each GitHub Release (`spring-voyage-agent-sidecar-<v>-{linux-amd64,linux-arm64,darwin-arm64}`) into the image and setting it as the `ENTRYPOINT`. (Amended 2026-05-13: an `npm install -g @cvoya/spring-voyage-agent-sidecar` alternative was previously documented; the npm package is no longer published, so the SEA binary is the canonical path-2 install method.) | Non-Debian distro, rootless image with non-default UIDs, or you can't have Node in the runtime layer. |
...
- **Release shape.** Bridge releases are cut on tags shaped `agent-base-vX.Y.Z` (amended 2026-05-12 to `spring-voyage-vX.Y.Z` — see [#2229](https://github.com/cvoya-com/spring-voyage/issues/2229)). The release workflow publishes the OCI image, the npm package, and the SEA binaries in lockstep so all three paths advance together. (Amended 2026-05-13: the npm-package publish step is dropped; the OCI image and SEA binaries continue to ship in lockstep on every release. BYOI conformance path 2 is now SEA-binary-only — see `docs/guide/operator/byoi-agent-images.md` path 2.)
