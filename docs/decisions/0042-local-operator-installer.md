...
> **Amended 2026-05-13.** The three separate operator-facing release assets — `spring-voyage-<v>-bundle.tar.gz`, `spring-voyage-dispatcher-<v>-<rid>.{tar.gz|zip}`, and `spring-<v>-<rid>.{tar.gz|zip}` — have been collapsed into a single per-RID host archive `spring-voyage-<v>-<rid>.{tar.gz|zip}` containing `bundle/`, `cli/`, and `dispatcher/` subdirectories. Operators download one file instead of three; the installer's checksum verification covers one entry rather than three. Filename references elsewhere in this ADR (notably § 7) should be read as subdirectories inside the unified archive. The 5-RID matrix and the `~/.local/bin/spring` symlink are unchanged. Sidecar SEA binaries remain a separate set of top-level release assets — BYOI agent-image extenders consume them directly and never run `install.sh`.
...
> **Amended 2026-05-13.** The three operator-facing assets the installer downloaded (`spring-voyage-<v>-bundle.tar.gz` + `spring-voyage-dispatcher-<v>-<rid>.{tar.gz|zip}` + `spring-<v>-<rid>.{tar.gz|zip}`) are now a single per-RID host archive `spring-voyage-<v>-<rid>.{tar.gz|zip}` with `bundle/`, `cli/`, and `dispatcher/` subdirectories. The release-version surface is still single — only the asset layout collapsed. The same release also introduces `install-<v>.sh`, a version-baked companion to the unversioned `install.sh`: it is byte-identical except that its `BAKED_VERSION` is filled in at release time so the script refuses to install any other version. The unversioned `install.sh` remains the canonical `curl … releases/latest/download/install.sh | bash` entry point; the versioned alias exists for docs / runbooks / CI that need to pin to a specific release without depending on `--version`. The same amendment fixed a latent tag-handling bug: pre-#2229 `install.sh` normalised the resolved tag by prepending `v`, which produced the non-existent `vspring-voyage-v…` URL after the tag rename and meant the unversioned installer had been silently broken against every release since #2229. The new resolver accepts `1.0.0`, `v1.0.0`, or `spring-voyage-v1.0.0` and normalises to `TAG=spring-voyage-v${SEMVER}`.
