...
**2.3 No Enterprise Grid / org-level install.** Standard workspace install only. The binding flow inspects `team.info` (or the OAuth response) at install time; if the workspace is part of an Enterprise Grid, binding is refused with a structured error (`SlackEnterpriseGridUnsupported`). Grid changes the auth model in ways the v0.1 connector does not handle, so explicit refusal is preferable to silent partial-functionality.
...
**Primary SV human selection.** In v0.1 OSS, every `Human` defaults to mapping onto `OssTenantUserIds.Operator` (ADR-0047 §7). The "primary" of that mapping is configured per tenant on the `TenantUser` row (a new nullable `primary_human_id` field). The OSS bootstrap pins it to whichever `Human` row was created first for the operator; tenant admins can update it via the CLI / portal user-identity surface. The forward-compatible alternative — match by identity attribute (e.g., the SV human whose email matches the signed-in Slack user's email) — is the multi-tenant default (decision 7.4); OSS uses the explicit pin because every SV human shares one email in OSS by construction.
...
**7.4 Primary-SV-human resolution is a function of `(tenant_user, slack_user_id)`.** The lookup that drops one SV human from the slug (decision 4) calls a function `ResolvePrimarySvHuman(tenant_user_id, slack_user_id)` that returns one `Human.Id`. In OSS v0.1 the implementation reads the explicit `TenantUser.primary_human_id` pin. In multi-tenant the implementation defaults to matching by email (`Human.Email == slack_user.email`), falling back to the explicit pin if matching fails. Both implementations satisfy the same signature; the slug-builder does not branch on deployment mode.
