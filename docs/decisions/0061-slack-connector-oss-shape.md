...
**2.4 Unbound-user refusal.** If a workspace member other than the bound user DMs the bot, the bot replies once with `"This Spring Voyage install is bound to one Slack user. You don't have access."` and ignores subsequent messages from that user. The refusal is observable (audit log), not silent.

> **Amended 2026-05-28 ([#2894](https://github.com/cvoya-com/spring-voyage/issues/2894)).** The refusal text is **generic** — it does not interpolate the installer's display name. It was originally specified here as `"…bound to <installer-display-name>. You don't have access."`. Naming the installer would leak the bound operator's identity to any workspace member who DMs the bot to probe it; in a self-hosted, single-bound-user install that information leak outweighs the marginal "who do I ask for access" benefit, so the safe-by-default generic string wins. The shipped constant `SlackEventDispatcher.UnboundUserRefusalText` (reused by `SlackCommandDispatcher.DmOnlyRefusalText` for the out-of-DM command refusal — decision 5) has always carried the generic string; this amendment aligns the ADR with the shipped code rather than the reverse.
