...
- The DI singletons (`McpServer`, `IMcpServer`, `PersistentAgentRegistry`, `AgentVolumeManager`, `EphemeralAgentRegistry`) stay registered in **both** hosts via `TryAddSingleton`, so any code path or OpenAPI doc-gen that resolves them still composes. This is not the end state — the API host still resolves these because `AgentEndpoints` / `UnitEndpoints` call into `PersistentAgentLifecycle` in-process. Removing the duplicate registration requires those endpoints to delegate to the worker; that is a tracked Wave 3 item ([#2618](https://github.com/cvoya-com/spring-voyage/issues/2618)) — see §3 and the Consequences.
...
This keeps `DeployAsync` callable from the API host for now. The container launch it performs goes through `DispatcherClientContainerRuntime`, a thin HTTP client to `spring-dispatcher` — a stateless forward, not in-process execution state, and therefore not the same class of "two of everything" defect as the duplicated `McpServer` session store. But it is still execution work in the wrong host: the API host registers and resolves `PersistentAgentLifecycle` (and transitively `McpServer`, `AgentVolumeManager`, `ContainerLifecycleManager`) only because `AgentEndpoints` / `UnitEndpoints` expose `/deploy` / `/undeploy` / `/deployment` / `/logs` and call into it in-process.

**Wave 1 does not finish this.** The target end state is `spring-api` as a *genuine* stateless front door: those endpoints delegate to the worker, and the `HttpFrontDoor` composition then registers **none** of the execution singletons — the duplicate `TryAddSingleton` registrations §1 preserves disappear. Wave 1 deliberately does not do it because (a) it is not load-bearing for fixing #2612 — the auto-start dispatch path already runs worker-side — and (b) folding an endpoint-delegation refactor into the correctness-critical PRs of Wave 1 trades unblocking the dogfooding loop against scope risk. It is a *simplification*, not a *correctness* fix, and is therefore a planned **Wave 3** item, tracked as [#2618](https://github.com/cvoya-com/spring-voyage/issues/2618) — not an open-ended "maybe later" follow-up.
...
- **The API host still registers the execution DI singletons — acknowledged, tracked debt, not residue.** Wave 1 gates the execution *hosted services* worker-only but leaves `PersistentAgentLifecycle` / `McpServer` / `AgentVolumeManager` / `EphemeralAgentRegistry` / `PersistentAgentRegistry` resolvable in the API host, because `AgentEndpoints` / `UnitEndpoints` call into them in-process. The duplication is removed for real in **Wave 3 / [#2618](https://github.com/cvoya-com/spring-voyage/issues/2618)**: the API persistent-agent endpoints delegate to the worker, after which `AddCvoyaSpringDapr(role: HttpFrontDoor)` registers none of these singletons. (`PersistentAgentRegistry` carries one nuance — it is also the EF-backed deployment-status table; a pure status read is legitimate front-door work, so #2618 decides whether the read-side stays API-resolvable.)
- **Wave 2 (#2608 workspace mount, #2609 terminology, #2610 dead code) and Wave 3** are unaffected by this ADR and proceed on their own tracks. Wave 3 collects the simplification items: serving the MCP surface as a route on the worker's existing Kestrel host instead of a standalone `HttpListener`; the API-host execution-services shed ([#2618](https://github.com/cvoya-com/spring-voyage/issues/2618)); re-evaluating whether `spring-dispatcher` still needs to be a separate host process. It is a natural successor — once the McpServer is unambiguously worker-only, folding its `HttpListener` into the worker's Kestrel pipeline and shedding the API host's residual execution singletons are both localised changes.
