...
> **Amendment (2026-05-21, [#2578](https://github.com/cvoya-com/spring-voyage/issues/2578)) — orchestration tools removed; delivery is messaging-only.** [ADR-0050](0050-platform-mcp-tool-surface.md) supersedes the §3–4 orchestration-tool surface: `delegate_to` / `fanout_to` are **deleted**. The platform has no orchestration tools — delivery is the two `sv.messaging.send` / `sv.messaging.multicast` tools (ADR-0049 contract), "delegation" is message *content* the recipient's runtime interprets, and recording an `OrchestrationDecision` is an optional `sv.runtime.report_decision` call rather than a delivery side effect. Treat the §3 / §4 orchestration-tool prose, table rows, and gates below as historical.
...
|---|---|---|---|
| ~~`list_members`~~ | ~~Enumerate the caller's own direct members …~~ **Superseded ([#2537](https://github.com/cvoya-com/spring-voyage/issues/2537)) — use `sv.directory.list_members` on the directory tool surface.** | | |
| ~~`inspect <address>`~~ | ~~Return metadata for any addressable target …~~ **Superseded ([#2537](https://github.com/cvoya-com/spring-voyage/issues/2537)) — use `sv.directory.get_member`.** | | |
...
| ~~`query_status <address>`~~ | ~~Cheap status check for a target …~~ **Superseded ([#2537](https://github.com/cvoya-com/spring-voyage/issues/2537)) — use `sv.directory.get_status`.** | | |
