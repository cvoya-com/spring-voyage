...
Two natural-language surfaces use this, both **engine-mediated** (the engine provides the context and executes the result — Claude never reaches *into* the orchestrator): **intake** (a fresh brief → a `{theme, slots}` plan, above) and **in-progress commands** (a director message on a running edition that matched no pending response → the engine hands Claude the edition state and executes the action Claude returns: `reply` to answer a status question *from that state*, or `approve` / `revise` on a sign-off). Deterministic keyword routing remains the fallback when no completion is wired, so the flow still runs headless. Letting Claude call *into* the engine through orchestrator-hosted local tools — for live `get_status` / `cancel` / `modify` rather than a pre-enumerated action set — is the next increment ([#3078](https://github.com/cvoya-com/spring-voyage/issues/3078)).
