...
### 6. Claude manages the orchestration through the engine's tools

A director's brief arrives as free-form natural language ("do a deep issue on city hall this month"); a deterministic graph cannot interpret it. The split is **data plane vs control plane**, and the engine is the front-line A2A receiver of both:

- **Data plane — the engine runs it.** A reply whose `in_reply_to` matches an outstanding delegation (the correlation map of §5) advances that slot's graph **autonomously**: deliver the next stage's brief, or finish the slot and — once every slot is packaged — assemble and ask the director to sign off. This high-frequency, structured traffic never touches an LLM.
- **Control plane — Claude manages it.** Every **unmatched** message (a director control message) is handed to **Claude**, invoked *with the engine's tools*. Claude reads the message and calls the tools to manage the orchestration; its reply is the agent's response.

The engine **co-hosts a local MCP server** exposing six tools — `start_edition(theme, slots, report_to) → edition_id`, `get_status`, `active_editions`, `cancel_edition`, `approve_edition`, `revise_edition` — backed directly by the coordinator. The SDK invokes `claude --print --mcp-config <config>` pointed at that localhost server, so Claude calls the tools mid-turn (re-entrantly — the engine's receive is suspended on the Claude turn while the tool server, a concurrent task, services the calls). Claude *manages* the orchestration; the engine *runs* it. The premise stays honest: a workflow engine orchestrates; the LLM is the natural-language front door that drives it, not the orchestrator.

**`edition_id` is engine-minted** — a fresh UUID per `start_edition`, **decoupled from the internal SV `thread_id`** (which is participant-set-stable and never a per-edition key — the trap that made an earlier draft run one edition for the agent's entire life; see [#3079](https://github.com/cvoya-com/spring-voyage/issues/3079)). Multiple editions therefore coexist, "reset" is just minting a new id, and Claude discovers what is running via `active_editions` rather than storing ids. Because the engine no longer receives the director's kickoff directly (Claude does, via the tool), it reaches the director with a **new `send_message([report_to])`**, not `respond_to`; the director's approve/revise reply then lands unmatched and routes back through Claude.

Because the co-hosted CLI authenticates exactly like the claude-code runtime, the `a2a-process → anthropic` catalogue edge is **OAuth** (`authMethod: oauth`): the launcher resolves the Claude OAuth token and injects `CLAUDE_CODE_OAUTH_TOKEN` — the same secret every agent in an OAuth deployment already uses, not an Anthropic API key. Resolution is **best-effort**: a purely deterministic engine that never invokes Claude boots with no credential at all (the engine image still co-hosts the CLI, which simply goes unused).
