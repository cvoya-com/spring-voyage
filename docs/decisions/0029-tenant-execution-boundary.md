...
- **Stage 0 (immediate, not V2.1 — unblocks the ephemeral dapr-agent today).** Drop the Dapr-Workflow execution wrapper from `agents/spring-voyage-agent/agent.py`. Keep the agentic loop as plain Python (reimplemented, borrowed from `dapr_agents`' lower-level classes, or substituted from another library). An ephemeral agent is defined by container-lifetime-tied-to-completion-of-work, not by any specific loop implementation.
