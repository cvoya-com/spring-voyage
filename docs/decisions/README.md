...
| [0038](0038-agent-runtime-and-model-provider-split.md) | AgentRuntime and ModelProvider as separate identities | Proposed — three-concept split (`AgentRuntime`, `ModelProvider`, `Model{provider,id}`); user-facing config collapses to `(runtime, model)`; both runtimes and providers as data in checked-in `runtime-catalog.yaml`; per-provider and per-runtime classes replaced by small strategy registries (`IAgentRuntimeLauncher`, `IModelProviderAdapter`); `Kind` removed; credentials re-key to `(tenant, provider, authMethod)` with unit-level inheritance per ADR-0003; provider picker shown only for non-fixed-provider runtimes; clean-deploy hard rename |
