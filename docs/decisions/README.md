...
| [0038](0038-agent-runtime-and-model-provider-split.md) | AgentRuntime and ModelProvider as separate identities | Proposed — three-concept split (`IAgentRuntime`, `IModelProvider`, `Model{provider,id}`); user-facing config collapses to `(runtime, model)`; provider config in checked-in `model-providers.yaml`; small set of wire-format adapters in place of per-provider classes; `Kind` removed; credentials re-key to `(tenant, provider, shape)`; clean-deploy hard rename |
