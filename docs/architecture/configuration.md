...
6. **`DispatcherConfigurationRequirement`** (`Cvoya.Spring.Dapr`, PR 2). `IsMandatory=true` on every host that runs `PersistentAgentRegistry` as a hosted service (the API and Worker hosts; design-time tooling skips the validator entirely). Validates `Dispatcher:BaseUrl` is a well-shaped absolute HTTP(S) URI and warns when `Dispatcher:BearerToken` is empty. Missing `BaseUrl` on a mandatory host → `Invalid` with a fatal error (replaces the silent-then-thrashing failure mode of issue #2518, where the API host crashed the restart loop and DELETEd `persistent_agent_runtime` rows). Malformed URL → `Invalid`. The mandatory flag is set at registration time via `DispatcherConfigurationRequirementOptions`; test harnesses that explicitly opt out of dispatcher-driven supervision may register with `IsMandatory=false`.
