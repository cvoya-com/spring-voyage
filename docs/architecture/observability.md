...
| `OrchestrationToolHandlers.HandleDelegateToAsync / HandleFanoutToAsync` | `DecisionMade` | a `delegate_to` / `fanout_to` from a unit's runtime — carries the target(s); `CorrelationId` is the thread id, so it joins the same correlation chain as the connector `MessageReceived` and the coordinator's routing-outcome `DecisionMade` (#2560) |
