...
**Addressing a unit** (rather than a specific member) sends the message to the unit actor. The unit applies its boundary filtering and invokes its own runtime through the same launcher path used for any agent ([ADR-0039](../decisions/0039-units-are-agents.md)). The launcher unconditionally attaches the `sv.messaging.send` / `sv.messaging.multicast` delivery tools to every `agent://` and `unit://` runtime ([ADR-0050](../decisions/0050-platform-mcp-tool-surface.md)); discovery, inspection, and status queries use the `sv.directory.*` tool surface. The runtime's instructions decide whether to answer directly or pass the work on.

**Nested dispatch.** A unit's members may themselves be units. Because `IUnitActor` inherits the shared `IAgent` mailbox contract, a parent unit's runtime may send work to a sub-unit through `sv.messaging.send` without branching on scheme — `IAgentProxyResolver` maps the scheme to the right actor type and the sub-unit runs its own runtime turn. Membership-graph depth is bounded to 64 levels; see [Units — Nested Units](units.md#nested-units-units-as-members-of-units) for membership invariants and cycle-detection semantics.
