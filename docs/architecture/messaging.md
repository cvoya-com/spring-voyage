...
**Addressing a unit** (rather than a specific member) sends the message to the unit actor. The unit applies its boundary filtering and invokes its own runtime through the same launcher path used for any agent ([ADR-0039](../decisions/0039-units-are-agents.md)). The launcher unconditionally attaches the `sv.messaging.send` / `sv.messaging.broadcast` delivery tools to every `agent://` and `unit://` runtime ([ADR-0050](../decisions/0050-platform-mcp-tool-surface.md)); discovery, inspection, and status queries use the `sv.directory.*` tool surface. The runtime's instructions decide whether to answer directly or pass the work on.
