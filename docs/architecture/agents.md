...
**Connector context (Layer 1, [#2442](https://github.com/cvoya-com/spring-voyage/issues/2442)).** When the dispatch subject has connector bindings (direct or inherited), the platform layer additionally renders an auto-injected subsection — `## Connector context (auto-injected by platform)` — built from each connector's `IConnectorPromptContextContributor.GetPromptHintsAsync(...)`. The contributor returns a markdown fragment naming the bound resource (e.g. `### GitHub binding — cvoya-com/spring-voyage`) and telling the agent which env-vars its container has and how to use them. The section is omitted entirely when no bindings apply or when every contributor returns `null`. See [`docs/architecture/agent-runtime.md` § 4h](agent-runtime.md#4h-connector-prompt-context-contribution-2442) for the resolver mechanics and the GitHub-side fragment shape.
...
**Sender rendering in prior turns ([#2129](https://github.com/cvoya-com/spring-voyage/issues/2129)).** Each prior message in Layer 3 is formatted as `[ts] {Sender}: {text}`. `{Sender}` is the resolved display name of the message's `From` address (via `IParticipantDisplayNameResolver`, lifted to `Cvoya.Spring.Core.Security` so the in-process prompt-assembly path can call it without depending on the API host). When resolution fails, the formatter falls back to the bare scheme literal (`human` / `agent` / `unit`) — never the raw `scheme://<guid>` wire form, which weak LLMs were observed mimicking on output ([#2089](https://github.com/cvoya-com/spring-voyage/issues/2089)). The Layer 1 platform prompt additionally instructs the model not to echo the timestamp / sender prefix from the conversation history, as a belt-and-braces complement to the input-side scrub.
