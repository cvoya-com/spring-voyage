...
- **The dispatcher runs as a host process** (issue [#1063](https://github.com/cvoya-com/spring-voyage/issues/1063)). It invokes the local `podman` binary directly rather than reaching a bind-mounted socket from inside a container. Running on the host removes the rootless-socket passthrough that fails on macOS arm64 / libkrun and gives Linux/macOS/Windows a single topology. The dispatcher is owned by [`eng/deploy/spring-voyage-host.sh`](../../eng/deploy/spring-voyage-host.sh).
- **.NET 10 SDK on the dispatcher host.** Direct `spring-voyage-host.sh start` publishes `Cvoya.Spring.Dispatcher` once and reuses the published binary on subsequent starts. Source-clone `deploy.sh up` calls `spring-voyage-host.sh restart --rebuild`, so every deploy republishes the dispatcher before the worker starts.
...
Workflow-driven runtime images (units whose `ai.runtime` selects a workflow launcher) follow this pattern for every dispatch — see [Workflows](workflows.md#workflow-as-container-primary-model). Agents whose runtime is `spring-voyage` use the same sidecar + dual-attach path for both ephemeral and persistent hosting so the in-container Dapr agent can run workflows. Other runtimes (for example Claude Code) use a detached app container without a per-container `daprd`, speak A2A to the dispatcher, and reach platform services via the host-level MCP server.
...
1. A maintainer runs `eng/release/release.sh v1.0.0` (or `--pre alpha|beta|rc` for a pre-release). The script pushes a single `spring-voyage-v<version>` tag.
2. `.github/workflows/release.yml` (triggered by the tag) builds the agent-base, OSS, and platform images as multi-arch manifests via `docker/build-push-action`. Other images use `eng/build/build-agent-images.sh --ghcr-only` (single-arch for v0.1).
3. The workflow pushes the immutable release tag. Stable releases also update the floating `:X.Y` and `:latest` tags.
4. GHCR packages are publicly pullable on first publish via the `cvoya-com` org's "Inherit access from source repository" setting plus the `org.opencontainers.image.source` OCI label every image carries.
...
Consumers that want the Dockerfile's `--build-arg CLAUDE_CODE_VERSION=<x.y.z>` escape hatch can continue to build locally; `eng/build/build-agent-images.sh` still tags the result with the same canonical GHCR ref in the local image store so the dispatcher finds it before pulling.
...
- **`Cvoya.Spring.Dapr`** — Dapr implementations: actors (`AgentActor`, `UnitActor`, `ConnectorActor`, `HumanActor`), `A2AExecutionDispatcher`, the Dapr `IOrchestrationToolProvider` implementation, `ExecutionConfigInheritanceResolver`, `PersistentAgentRegistry`, `DaprStateBackedSecretStore`, state management, and routing.
...
- **`eng/dapr/`** — Dapr component configuration (pub/sub, state, bindings, secrets, resiliency).
