...
An `ActivityEvent` with `EventType=DecisionMade` published by the platform each time a unit's runtime calls a delegation tool. Shape: `Kind` (`Delegate` | `Fanout` | `Inspect` | `NoOp`), `Status` (`Accepted` | `Routed` | `Failed`), `Targets` (array of child unit addresses), `ResultMessageIds` (array of result message Guids), `Reason` (optional runtime-supplied string).
