...
The install command shape-checks `--sv-host` before contacting Slack. It rejects loopback URLs without an explicit port (`http://localhost` resolves to port 80, which is almost never where the dev API listens) and warns when the host is a loopback or non-HTTPS — both signals that the install would land a Slack app whose registered URLs the workspace can't actually reach. Pass `--sv-host` explicitly or fix `Endpoint` in `~/.spring/config.json` to silence the check.
...
[Socket Mode](https://api.slack.com/apis/socket-mode) lets a Slack app receive events, slash commands, and interactions over a long-lived WebSocket the bot opens outbound to Slack. The events / commands / interactions URLs in the manifest are ignored while Socket Mode is on, so they can stay pointed at `http://localhost:5000` without breaking anything. The OAuth callback still needs to be reachable from the operator's browser — `http://localhost:5000` works for that out of the box.
...
```bash
# Terminal 1 — the Spring Voyage API
...
Pick one — any tool that exposes a stable HTTPS URL pointed at `http://localhost:5000` works:

- [ngrok](https://ngrok.com/) — `ngrok http 5000` prints a `https://<random>.ngrok.app` URL.
