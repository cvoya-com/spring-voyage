...
- [Path A — Create from manifest (recommended)](#path-a--create-from-manifest-recommended) — paste a single YAML blob into Slack's "Create an app from a manifest" form. The manifest pre-configures every scope, URL, slash command, and event subscription so the operator clicks through one screen rather than seven.
- [Path B — Manual registration](#path-b--manual-registration) — point-and-click on api.slack.com if you want to inspect every field.
...
## Path A — Create from manifest (recommended)

Slack's [manifest API](https://api.slack.com/reference/manifests) lets you create an app from a single YAML document. This is the shortest path: paste the blob below into Slack's "Create an app from a manifest" form, click through one confirmation screen, and capture the three credentials.
...
Whichever path you took, the deployment ends up with these three values populated in `eng/config/spring.env`:
...
Open `eng/config/spring.env` and add the Slack block:
...
Follow [Path A](#path-a--create-from-manifest-recommended) or [Path B](#path-b--manual-registration) but substitute `<tunnel-host>` everywhere the doc says `<your-host>`. Set the redirect URL, event request URL, interaction request URL, and slash-command URLs all to your tunnel hostname.
