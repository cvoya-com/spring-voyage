...
- [Slack app setup](slack-app-setup.md) — per-deployment Slack app registration (OAuth, scopes, event + slash-command URLs, manifest YAML).
- [BYOI agent images](byoi-agent-images.md) — bringing your own agent container images.
- [Model providers](model-providers.md) — installing and configuring per-tenant model providers (Anthropic, OpenAI, Google, Ollama).
