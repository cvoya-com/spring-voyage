...
4. Clicking **Next** on Step 1 automatically validates the typed key (#655). The wizard posts it to `POST /api/v1/system/credentials/{provider}/validate`, which performs a lightweight read-only call against the provider's own API (`GET /v1/models` for Anthropic/OpenAI, `GET /v1beta/models` for Google). On success the Next button advances to the next step and the Model dropdown is re-seeded from the returned list so operators pick from what their account actually supports rather than the static fallback. On failure the wizard stays on Step 1 and the error message is surfaced inline under the credential input. Editing the key clears the verdict so the next Next click re-validates.
