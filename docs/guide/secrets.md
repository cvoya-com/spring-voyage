...
4. After typing a key, click **Validate** (#655). The wizard posts the key to `POST /api/v1/system/credentials/{provider}/validate`, which performs a lightweight read-only call against the provider's own API (`GET /v1/models` for Anthropic/OpenAI, `GET /v1beta/models` for Google). On success the Model dropdown is re-seeded from the returned list so operators pick from what their account actually supports rather than the static fallback. Validation gates the wizard's Next button — a typed-but-unvalidated key is blocked, a validation error surfaces inline, and editing the key clears the verdict so the operator re-validates.
