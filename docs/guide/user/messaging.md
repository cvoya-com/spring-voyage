...
- [`messaging/agent-domain-message.sh`](../../../tests/e2e/cli/scenarios/messaging/agent-domain-message.sh) — sends a Domain message to an agent and verifies the `MessageArrived` activity event lands. Proves the router → actor → activity-bus path end-to-end.
- [`messaging/conversation-lifecycle.sh`](../../../tests/e2e/cli/scenarios/messaging/conversation-lifecycle.sh) — starts a fresh conversation on an idle agent and verifies the three lifecycle events fire in order: `MessageArrived` → `ThreadStarted` → `StateChanged (Idle→Active)`.
