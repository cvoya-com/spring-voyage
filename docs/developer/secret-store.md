...
> **`agent: spring-voyage, provider: anthropic` is not yet wired in OSS.** The Dapr-Agent runtime (`spring-voyage` tool) talks to the LLM through Dapr Conversation components. The OSS deployment ships only `conversation-ollama.yaml` (the tenant-network Ollama instance); there is no `conversation-anthropic.yaml`, and the `SpringVoyageAgentLauncher` does not propagate `ANTHROPIC_API_KEY` into the container today. An operator who configures `agent: spring-voyage, provider: anthropic` will see the unit deploy, but the agentic loop's first `DaprChatClient.generate` call will fail because the `llm-provider` Dapr component has no Anthropic backend wired. Tracked in #1714 — covers the per-provider Conversation-component work and `SpringVoyageAgentLauncher` env-injection.
