...
The umbrella `spring-voyage-oss` unit is the sole holder of the GitHub connector binding. The `owner`, `repo`, and `installation_id` fields are intentionally absent from the checked-in package YAML — they require per-deployment identity that does not belong in source. The operator supplies them as inputs when running `spring package install spring-voyage-oss` (or through the **Catalog** path in the New Unit wizard), and the platform creates the unit hierarchy and the umbrella's binding atomically as part of the install. Sub-units do not bind the connector themselves; they inherit `$GITHUB_TOKEN` and the other GitHub env vars from the umbrella's binding via the platform's connector binding-walk (closest binding in the parent chain wins). See [Connectors](connectors.md) for the binding model and the GitHub connector's repository and reviewer discovery behaviour.

## How GitHub events reach the OSS unit

A GitHub webhook arrives at the platform's `/api/v1/webhooks/github` endpoint. The GitHub connector validates the HMAC signature against the App's webhook secret, parses the payload, and translates the event into a domain message with a structured `intent` field (`work_assignment`, `assignment`, `label_change`, `feedback`, `review_result`, …) plus the typed event-specific payload. The translator then resolves the destination: it matches the webhook's `(installation_id, owner, repo)` triple against the platform's connector bindings and rewrites the message's `To` address to the unit that owns the matching binding — for this package, always the umbrella `spring-voyage-oss`. Any inbound filters declared on the binding (label, author, path) are evaluated; matching events are dropped with an audit entry.

The umbrella unit's runtime receives the resulting A2A message on its mailbox. Because the umbrella has members, the platform attaches a closed five-tool orchestration MCP surface (`list_children`, `inspect_child`, `delegate_to_child`, `fanout_to_children`, `query_child_status`) at launch time. The runtime uses those tools to dispatch the message — or a derived task — to whichever sub-unit owns the work, which in turn delegates to its engineer or PM instances. Each delegation hop emits an `OrchestrationDecision` activity event for audit.
