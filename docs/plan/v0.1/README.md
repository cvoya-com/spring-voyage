...
- **[#2487](https://github.com/cvoya-com/spring-voyage/issues/2487) — Platform-user / human split + connector identity location.** Introduces the `PlatformUser` actor kind, relocates connector-side user identity off the unit binding and off `Human` rows onto the platform user, simplifies the GitHub binding to `(owner, repo)`-only, and ships OAuth-issued PAT acquisition as a v0.1 in-scope flow. Supersedes the original single-PR shape of #2487 (a localised webhook fix) and pulls forward platform-user pieces originally scoped for v0.2 / v0.3. Locked in [ADR-0047](../../decisions/0047-platform-user-human-split.md); execution plan in [`platform-user-split.md`](platform-user-split.md).
