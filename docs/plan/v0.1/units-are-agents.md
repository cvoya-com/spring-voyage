...
> **Amendment (2026-05-21, [#2578](https://github.com/cvoya-com/spring-voyage/issues/2578), [ADR-0050](../../decisions/archive/0050-platform-mcp-tool-surface.md)) — orchestration tools removed; delivery is messaging-only.** The `delegate_to` / `fanout_to` orchestration tools are **removed outright** — the platform delivers messages, it does not orchestrate. The whole Phase D orchestration-tool surface (`delegate_to_child` / `fanout_to_child` and their later `delegate_to` / `fanout_to` renames, `OrchestrationToolName`, the REST `/delegate-to` `/fanout-to` routes, the SDK delegate/fanout client) is superseded. The platform's delivery surface is now exactly two MCP tools — `sv.messaging.send` / `sv.messaging.multicast` — on the [ADR-0049](../../decisions/archive/0049-message-delivery-tool-contract.md) delivery-acknowledgement contract. Every platform MCP tool is renamed into the `sv.<area>.<verb>` taxonomy (areas: `directory`, `memory`, `messaging`, `runtime`, `expertise`). "Delegation" is now message *content* the recipient's runtime interprets; recording a routing decision is an optional `sv.runtime.report_decision` call (generalised to record any routing decision, executed or not). The per-thread depth budget (D8 below) survives as a per-thread hop counter incremented on every `sv.messaging.*` call. The current narrative lives in [`docs/architecture/messaging.md`](../../architecture/messaging.md); treat all Phase D tool-surface prose below as historical.
>
...
**Initiative.** Implementation of [ADR-0039 — Units are agents (orchestration is runtime behaviour, not platform configuration)](../../decisions/archive/0039-units-are-agents.md). Tracked under [#1786](https://github.com/cvoya-com/spring-voyage/issues/1786). Subsumes the agent-create UX redesign tracked under [#1763](https://github.com/cvoya-com/spring-voyage/issues/1763); the design contract for the UX-side tasks lives at [`docs/design/v0.1/agent-create-redesign.md`](../../design/v0.1/agent-create-redesign.md).
...
- **Deliverable.** REST endpoints — one per orchestration tool — under a base path the dispatcher controls (e.g. `/v1/runtime/orchestration/list-children`, `/inspect-child`, `/delegate-to-child`, `/fanout-to-children`, `/query-child-status`). Each endpoint applies, in order:
  ...
  Endpoints are not part of the public OpenAPI surface — they are runtime-internal callbacks; document them in `docs/architecture/agent-runtime.md` (D18).
...
**D18 — Documentation: `docs/architecture/agent-runtime.md`**

- **Files:** `docs/architecture/agent-runtime.md` (new).
