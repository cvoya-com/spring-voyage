...
cmd_start() {
    ...
    (
        ...
        # repo's eng/dapr/components/delegated-spring-voyage-agent is the OSS source; do
        # not use /dapr/... (image-only) — Podman will fail with statfs (#1197 follow-up).
        # In the source-free bundle layout the same tree lives at
        # ${REPO_ROOT}/dapr/components/delegated-spring-voyage-agent, so fall back to that.
        _delegated_components="${REPO_ROOT}/eng/dapr/components/delegated-spring-voyage-agent"
        if [[ ! -d "${_delegated_components}" && -d "${REPO_ROOT}/dapr/components/delegated-spring-voyage-agent" ]]; then
