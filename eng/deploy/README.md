...
[`eng/build/`](../build/README.md) and `./deploy.sh`) and the source-free
...
under [`eng/build/`](../build/README.md). The TypeScript agent sidecar source
...
(built from [`eng/build/`](../build/README.md)) — pick the one that
...
```bash
./eng/build/build-agent-images.sh                # all seven at :dev
./eng/build/build-agent-images.sh --tag latest   # all seven at :latest
./eng/build/build.sh --ghcr-only                 # release-style canonical tags only
```

To layer extra tooling on top of one of the bases, start from a
template under [`eng/build/examples/dockerfiles/`](../build/examples/dockerfiles/):
...
Dapr components (`eng/dapr/components/production/*.yaml`) and the Dapr
Configuration (`eng/dapr/config/production.yaml`) are bind-mounted read-only
...
docker compose -f eng/deploy/docker-compose.yml up
```
...
```bash
bash eng/deploy/scripts/test-spring-voyage-host.sh    # 8 idempotence cases
bash eng/deploy/scripts/dispatcher-smoke.sh           # alpine echo round-trip
```
...
connection string. See eng/deploy/README.md.
```
...
and [`eng/dapr/README.md`](../dapr/README.md) for profile details.
