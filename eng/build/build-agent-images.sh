...
#
# Usage:
#   eng/build/build-agent-images.sh                # builds :dev tags
#   eng/build/build-agent-images.sh --tag 1.2.3    # builds :1.2.3 tags
#   eng/build/build-agent-images.sh --help
#
...
#                         gemini-base, and OSS role builds instead of the
#                         locally-built one. Lets CI verify the published
#                         image without rebuilding.
#
# Mirrors the structure and style of `eng/build/build-sidecar.sh` so an
...
usage() {
    cat <<EOF
Usage: eng/build/build-agent-images.sh [options]
...
Examples:
  ...
  eng/build/build-agent-images.sh --skip-oss
EOF
}
...
# ---- 3. gemini-base (path 1) ---------------------------------------------
# Mirrors claude-code-base: FROMs agent-base, npm-installs the Gemini CLI.
# The runtime catalogue points at this image (`eng/runtime-catalog/runtime-catalog.yaml`,
# `agentRuntimes[id=gemini].defaultImage`).
gemini_tags=(--tag "${GEMINI_IMAGE}:${TAG}")
if [[ "${TAG_LOCAL_ALIASES}" -eq 1 ]]; then
    gemini_tags+=(--tag "${GEMINI_LOCAL_ALIAS}:${TAG}")
fi

log "building ${GEMINI_IMAGE}:${TAG} (FROM ${AGENT_BASE_OVERRIDE})"
"${DOCKER}" build \
    --file "${SCRIPT_DIR}/Dockerfile.agent.gemini" \
    --build-arg "AGENT_BASE_IMAGE=${AGENT_BASE_OVERRIDE}" \
    "${gemini_tags[@]}" \
    "${REPO_ROOT}"
