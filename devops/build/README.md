...
install flow at `devops/install/` pulls these images from GHCR instead).
...
| `build-sidecar.sh`             | Builds `ghcr.io/cvoya-com/agent-base:dev` from `src/Cvoya.Spring.AgentSidecar/`.        |
| `Dockerfile`                   | Multi-stage platform image (.NET 10 API/Worker + Web + Dapr CLI).                       |
| `Dockerfile.agent-base`        | A2A bridge sidecar base image — published as `ghcr.io/cvoya-com/agent-base:<semver>`.   |
...
plus the seven agent images. See [`devops/deploy/README.md`](../deploy/README.md)
